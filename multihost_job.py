...
def get_project():
    ...
    if len(project_outputs) < 1 or project_outputs[-1] == "":
        sys.exit(
            "You must specify the project in the PROJECT flag or set it with 'gcloud config set project <project>'"
        )
...
def get_zone():
    completed_command = subprocess.run(
        ["gcloud", "config", "get", "compute/zone"], check=True, capture_output=True
    )
    zone_outputs = completed_command.stdout.decode().strip().split("\n")
    if len(zone_outputs) < 1 or zone_outputs[-1] == "":
        sys.exit(
            "You must specify the zone in the ZONE flag or set it with 'gcloud config set compute/zone <zone>'"
        )
...
def print_flags(args):
    ...
    print(
        f"Enable Autocheckpoint (--ENABLE_AUTOCHECKPOINT) = {args.ENABLE_AUTOCHECKPOINT}\n"
    )
...
def run_create_resources(startup_script_file, args):
    ...
    command = rf"gcloud alpha compute tpus queued-resources create {args.RUN_NAME} --accelerator-type={args.TPU_TYPE} --runtime-version={args.VERSION} --project={args.PROJECT} --zone={args.ZONE}"
    if args.NUM_SLICES > 1:
        command = (
            command + f" --node-prefix={args.RUN_NAME} --node-count={args.NUM_SLICES}"
        )
    else:
...
def write_startup_script(
    zip_gcs_path, zip_name, log_name, bucket_path, startup_script_file, args
):
...
PROJECT=$(grep '^CONSUMER_PROJECT_ID' /tmp/tpu-env | cut -d "'" -f 2)"""
    if num_slices == 1:
        ...
        slice_assignment = (
            """SLICE_ID=$(grep '^MEGASCALE_SLICE_ID' /tmp/tpu-env | cut -d "'" -f 2)"""
        )
    return env_str + "\n" + slice_assignment
...
def main(raw_args=None) -> None:
    ...
    parser.add_argument(
        "--TPU_TYPE", type=str, default="v4-8", help="The type of the TPU"
    )
    parser.add_argument(
        "--VERSION",
        type=str,
    ...
    if not args.RUN_NAME:
        args.RUN_NAME = (
            get_run_name()
        )  # Used for QR name, TPU_PREFIX, logging file, and tmp json file.
    ...
    captured_output = move_script_dir_to_gcs(
        args.SCRIPT_DIR, tmp_dir_relative_to_script, zip_name, bucket_path
    )
    ...
    print("------------------------------------ \n")
    print(
        "multihost_job finished running, TPUs are firing up now to run your job remotely.\n"
    )

    print(
        f"Your job is being logged, follow it here:\n{google_cloud_logging_url(args.RUN_NAME, args.PROJECT)}\n"
    )
