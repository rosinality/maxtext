...
def validate_kv_quant_axis(s: str, quantize_kvcache: bool) -> None:
    ...
    if s not in valid_kv_quant_axis:  # currently supported kv_quant_axis
        raise ValueError(
            "Invalid kv_quant_axis was passed. Valid options ", valid_kv_quant_axis
        )
...
def validate_profiler_type(s: str) -> None:
    ...
    if s not in valid_profiler_types:  # currently supported attention
        raise ValueError(
            "Invalid profiler type was passed. Valid options ", valid_profiler_types
        )
...
def validate_periodic_profiler(profiler, profile_periodically_period, profiler_steps):
    ...
    if not profiler:
        raise ValueError(
            "Periodic profiler requested but no profiler was set, set it via profiler=xplane or profiler=nsys"
        )
...
def validate_model_call_mode(s: str) -> None:
    ...
    if s not in valid_model_call_modes:  # currently supported attention
        raise ValueError(
            f"Invalid model call mode {s}. Valid options are {valid_model_call_modes}"
        )


def validate_prefill_and_target_lengths(
    max_prefill_length: int, max_target_length: int
) -> None:
    if max_prefill_length <= 0:
        raise ValueError(
            f"Invalid max_prefill_predict_length {max_prefill_length}, it should be a positive number"
        )
...
def validate_rope_type(rope_type: str) -> None:
    ...
    if rope_type not in valid_rope_types:
        raise ValueError(
            f"Invalid RoPE type was passed. Got: {rope_type}. Valid options: {valid_rope_types}"
        )
...
def validate_keys(keys):
    ...
    validate_periodic_profiler(
        keys["profiler"], keys["profile_periodically_period"], keys["profiler_steps"]
    )
    ...
    validate_prefill_and_target_lengths(
        keys["max_prefill_predict_length"], keys["max_target_length"]
    )
    validate_rope_type(keys["rope_type"])

    # TODO remove after b/435512699 resolved
    if (
        keys["context_parallel_size"] > 1
        and keys["context_parallel_load_balance"]
        and keys["attention_type"] == "chunk"
    ):
        raise ValueError(
            "Currently load-balanced context parallelism is not supported for chunk attention."
        )

    if keys["mtp_eval_target_module"] < 0:
        raise ValueError(
            "mtp_eval_target_module cannot be negative. Set to 0 to disable evaluation."
        )
    ...
    assert (
        keys["load_parameters_path"] == "" and keys["load_full_state_path"] == ""
    ) or keys[
    ...
    if keys["use_multimodal"]:
        ...
        if keys["use_sft"]:
            ...
            assert not keys[
                "packing"
            ], "In multimodal SFT (use_multimodal=True, use_sft=True), packing is not supported yet"
...
def validate_constant_bound(keys):
    ...
    assert (
        len(keys["constant_bound_config"]) == 0
        or len(keys["constant_bound_config"]) == 6
...
def validate_data_input(keys):
    ...
    if keys["dataset_type"] == "hf":
        ...
        assert (
            keys["num_epoch"] == 1
        ), f"hf pipeline only supports num_epoch=1, but num_epoch={keys['num_epoch']} is given."
        ...
        assert (
            keys["grain_train_files"] != ""
        ), "grain_train_files can't be empty when dataset_type=grain"
        if keys["eval_interval"] > 0:
            assert keys[
                "grain_eval_files"
        ...
        assert (
            keys["dataset_name"] != ""
        ), "dataset_name can't be empty when dataset_type=tfds"
        if keys["eval_interval"] > 0:
            assert keys[
                "eval_split"
...
def validate_model_name(s: str) -> bool:
    ...
    if s not in valid_model_names:
        raise ValueError(
            f"Invalid model name was passed. Got {s}, Valid options {valid_model_names}"
        )
...
def validate_and_assign_remat_tensors(keys):
    ...
    assert (
        keys["decoder_layer_input"] != "remat"
    ), "Cannot remeterialize this tensor with scan_layers=True"
    tensors_on_device = []
    tensors_to_offload = []
    for t in tensors:
        if keys[t] == "device":
...
class _HyperParameters:
    ...
    def _update_from_env_and_command_line(
        ...
        for k in raw_data_from_yaml:
            ...
            if new_proposal is None:
                raw_keys[k] = (
                    None  # This allows users to set empty strings via CLI, otherwise parsed as "None" - b/405981568
                )
        ...
        if _BASE_CONFIG_ATTR in raw_data_from_yaml:
            ...
            if not os.path.isabs(parent_config_filename):
                ...
                if not os.path.isfile(loaded_parent_config_filename):
                    ...
                    loaded_parent_config_filename = os.path.join(
                        dir_path, "configs", parent_config_filename
                    )
            else:
    ...
    def user_init(raw_keys):
        ...
        raw_keys["num_query_heads"] = (
            2**num_head_scale * raw_keys["base_num_query_heads"]
        )
        ...
        raw_keys["num_decoder_layers"] = (
            2**layer_scale * raw_keys["base_num_decoder_layers"]
        )
        ...
        if raw_keys["pagedattn_max_pages_per_group"] <= 0:
            raw_keys["pagedattn_max_pages_per_group"] = (
                raw_keys["max_target_length"]
                + raw_keys["pagedattn_tokens_per_page"]
                - 1
            ) // raw_keys["pagedattn_tokens_per_page"]

        raw_keys["num_slices"] = max_utils.get_num_slices(raw_keys)
        raw_keys["quantization_local_shard_count"] = get_quantization_local_shard_count(
            raw_keys
        )
        ...
        raw_keys["logical_axis_rules"] = _lists_to_tuples(
            raw_keys["logical_axis_rules"]
        )
    ...
    def configure_gpt3_task(raw_keys):
        ...
        decay_end_step = math.ceil(
            108600.0 * 1536 / global_batch_size_to_train_on - 1e-6
        )
    ...
    def configure_c4_mlperf_task(raw_keys):
        ...
        decay_end_step = math.ceil(
            1200000.0 * 1152 / global_batch_size_to_train_on - 1e-6
        )
        raw_keys["learning_rate"] = learning_rate
        raw_keys["learning_rate_schedule_steps"] = decay_end_step
        raw_keys["warmup_steps_fraction"] = warmup_steps / decay_end_step

        raw_keys["eval_steps"] = math.ceil(
            5760 * 8192 / max_target_length / global_batch_size_to_eval_on
        )
        raw_keys["eval_interval"] = math.ceil(
            377487360 / max_target_length / global_batch_size_to_train_on
        )
...
def set_mu_dtype(raw_keys):
    ...
    if raw_keys["mu_dtype"]:
        assert (
            raw_keys["opt_type"] != "adam_pax"
        ), "opt_type adam_pax doesn't support explicitly setting mu_dtype"
...
def validate_and_set_hlo_dump_defaults(raw_keys):
    ...
    if not os.environ.get("XLA_FLAGS") and not raw_keys["dump_hlo_xla_flags"]:
        raw_keys["dump_hlo_xla_flags"] = (
            f"--xla_dump_to={raw_keys['dump_hlo_local_dir']} --xla_dump_large_constants"
        )
    ...
    if not raw_keys["dump_hlo_gcs_dir"]:
        raw_keys["dump_hlo_gcs_dir"] = os.path.join(
            raw_keys["base_output_directory"], raw_keys["run_name"], "xla_dump"
        )
    else:
        raw_keys["dump_hlo_gcs_dir"] = gcs_utils.add_trailing_slash(
            raw_keys["dump_hlo_gcs_dir"]
        )
...
def set_and_validate_pipeline_config(raw_keys):
    if using_pipeline_parallelism(raw_keys):
        ...
        raw_keys["logical_axis_rules"] = modify_activation_embed_and_logits_batch(
            raw_keys["logical_axis_rules"]
        )
        raw_keys = pipeline_first_axis(raw_keys)
        num_stages = int(
            raw_keys["ici_pipeline_parallelism"] * raw_keys["dcn_pipeline_parallelism"]
        )
        if raw_keys["pipeline_parallel_layers"] == -1:
            if raw_keys["decoder_block"] == "deepseek":
                moe_layers = (
                    raw_keys["num_decoder_layers"] - raw_keys["first_num_dense_layers"]
                )
        ...
        assert (
            raw_keys["scan_layers"]
            or raw_keys["pipeline_parallel_layers"] == raw_keys["num_decoder_layers"]
        ...
        if raw_keys["num_pipeline_repeats"] == -1:
            ...
            assert not remainder, f"The number of layers per stage ({raw_keys['num_layers_per_pipeline_stage']}) times the number of stages ({num_stages}) must divide the number of pipeline_parallel_layers which defaults to decoder layers  ({raw_keys['pipeline_parallel_layers']}) "
            raw_keys["num_pipeline_repeats"] = num_pipeline_repeats
        assert (
            num_stages
            * raw_keys["num_pipeline_repeats"]
            * raw_keys["num_layers_per_pipeline_stage"]
            == raw_keys["pipeline_parallel_layers"]
        ...
        assert (
            raw_keys["micro_batch_size_to_train_on"]
            % raw_keys["num_pipeline_microbatches"]
            == 0
...
def validate_deepseek_moe(raw_keys):
    if raw_keys["n_routing_groups"] != -1:
        if raw_keys["topk_routing_group"] == -1:
            raise ValueError(
                f'config topk_routing_group: {raw_keys["topk_routing_group"]} is not defined'
            )
...
def validate_mlp_dim(raw_keys):
    ...
    is_fully_moe_model = (
        raw_keys["interleave_moe_layer_step"] == 1
        and raw_keys["first_num_dense_layers"] == 0
    )
...
def validate_sparse_matmul_parallelism(raw_keys):
    ...
    if raw_keys["sparse_matmul"] and (
        using_expert_parallelism(raw_keys) and using_pipeline_parallelism(raw_keys)
    ):
        raise ValueError(
    ...
    if (
        ...
        and using_tensor_parallelism(raw_keys)
        and (raw_keys["emb_dim"] % tensor_parallelism)
    ):
        raise ValueError(
            f"The embedding dimension {raw_keys['emb_dim']} is not divisible by tensor parallelism setting {tensor_parallelism}."
        )
    expert_parallelism = (
        raw_keys["ici_expert_parallelism"] * raw_keys["dcn_expert_parallelism"]
    )
    if (
        raw_keys["sparse_matmul"]
        and using_expert_parallelism(raw_keys)
        and (raw_keys["num_experts"] % expert_parallelism)
    ):
        raise ValueError(
...
def create_new_logical_axis_rules(old_logical_axis_rules, new_logical_axis_rules):
    ...
    for logical_axis, mesh_axes in new_logical_axis_rules:
        logical_axis_exists = any(
            rule for rule in old_logical_axis_rules if rule[0] == logical_axis
        )
...
def validate_and_update_keys(raw_keys, model_keys, config_name: str):
    ...
    for k in model_keys:
        ...
        if k not in raw_keys:
            ...
            raise ValueError(
                f"Type of key:{k} does not match with {type(model_keys[k])}"
            )
        else:
...
def calculate_global_batch_sizes(
    ...
    global_batch_size_to_load = int(
        micro_batch_size_to_load * gradient_accumulation_steps
    )
    global_batch_size_to_train_on = int(
        micro_batch_size_to_train_on * gradient_accumulation_steps
    )
    return (
...
def using_pipeline_parallelism(raw_keys) -> bool:
    return (
        int(raw_keys["ici_pipeline_parallelism"]) > 1
        or int(raw_keys["dcn_pipeline_parallelism"]) > 1
    )
...
def using_sequence_parallelism(raw_keys) -> bool:
    if (
        ...
        raise ValueError(
            "Expert parallelism can only be enabled on ICI or DCN, not both."
        )
    return (
        int(raw_keys["ici_sequence_parallelism"]) > 1
        or int(raw_keys["dcn_sequence_parallelism"]) > 1
    )
...
def using_expert_parallelism(raw_keys) -> bool:
    return (
        int(raw_keys["ici_expert_parallelism"]) > 1
        or int(raw_keys["dcn_expert_parallelism"]) > 1
    )
