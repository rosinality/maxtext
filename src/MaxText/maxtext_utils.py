...
def get_functional_train_with_signature(
    ...
    functional_train = functools.partial(
        train_step, model, config, state_mesh_shardings
    )
...
def calculate_mixed_attention_model_tflops_training_per_device(
    ...
    learnable_weight_tflops = (
        (
            (total_ffn_flops + qkv_flops + projection_flops) * num_layers
            + embedding_flops
        )
        * 3
        / 10**12
...
def calculate_llama4_attention_tflops(config):
    ...
    chunked_attention_flops_per_layer = _calculate_chunked_attention_flops_per_layer(
...
def calculate_mla_tflops_per_device(config):
    ...
    if config.q_lora_rank == 0:
        q_flops = (
            2 * batch_len * config.emb_dim * config.num_query_heads * qk_head_dim_sum
        )
    else:
    ...
    projection_flops = (
        2 * batch_len * config.emb_dim * config.num_query_heads * config.v_head_dim
    )
...
def calculate_routed_and_shared_ffn_tflops_per_device(config):
    ...
    dense_ffn_flops = (
        calculate_ffn_mamtul_tflops_per_device(config, config.mlp_dim)
        * num_dense_layers
    )
    shared_experts_flops = (
        calculate_ffn_mamtul_tflops_per_device(config, config.moe_mlp_dim)
...
def calculate_gemma3_vision_layers_tflops_per_device(config):
    ...
    projection_flops_per_layer = (
        2 * B * seq_len * hidden_dim * hidden_dim
    )  # projection after attention multiplication
    mlp_flops_per_layer = 2 * (
        2 * B * seq_len * hidden_dim * intermediate_dim
    )  # two fc layers
    total_attn_flops = attn_flops_per_layer * num_layers
    encoder_flops = (
        +qkv_flops_per_layer + projection_flops_per_layer + mlp_flops_per_layer
    ) * num_layers

    # 4. VisionEmbedder
    seq_len_after_pooling = (num_patches_h // vision_exit_pooling_window) * (
        num_patches_w // vision_exit_pooling_window
    )
    vision_embedder_flops = (
        2 * B * seq_len_after_pooling * hidden_dim * embed_dim
    ...
    if config.freeze_vision_encoder_params:
        learnable_weight_flops += (
            2 * vision_embedder_flops
        )  # only projector is learnable, add fwd+optimizer
    else:
...
def calculate_llama4_vision_layers_tflops_per_device(config):
    ...
    qkv_flops_per_layer = 3 * (
        2 * B * seq_len * hidden_dim * hidden_dim
    )  # Q, K, V projections
    attn_flops_per_layer = (
        4 * B * seq_len * seq_len * hidden_dim
    ...
    pixel_shuffle_fc1_flops = (
        2 * B * pixel_shuffle_tokens * intermediate_dim * pixel_shuffle_fc1_out_dim
    )
    pixel_shuffle_fc2_flops = (
        2
        * B
        * pixel_shuffle_tokens
        * pixel_shuffle_fc1_out_dim
        * pixel_shuffle_fc2_out_dim
    )
    ...
    projector_flops = (
        2 * B * pixel_shuffle_tokens * pixel_shuffle_fc1_out_dim * base_emb_dim
    )
...
def calculate_tflops_training_per_device(config, log=True):
    ...
    if config.attention_type == "mla":
        qkv_flops, noncausal_attention_flops, projection_flops = (
    ...
    if config.decoder_block == DecoderBlockType.GEMMA2:
        ...
        attention_tflops = (
            causal_attention_flops * config.num_decoder_layers * 3 / 10**12
        )
    else:
    ...
    if config.use_multimodal:
        ...
        mm_total_tflops, mm_learnable_weight_tflops, mm_attention_tflops = (
            calculate_vision_encoder_tflops(config)
        )
        if log:
            print(
...
def _get_nontrival_mesh_axes(mesh):
    ...
    return {
        axis
        for axis in target_sharding_axes_config
...
def _analyze_sharding(params, mesh, valid_target_mesh_axes):
    ...
    unsharded_params_total_size = (
        0  # Initialize a counter for the size of unsharded parameters.
    )
    ...
    for path, p_leaf in all_params_leaves:  # Iterate over each parameter leaf
        ...
        )  # Extract the current tensor's sharding spec
        ...
        if not is_sharded_on_all_target_axis:
            unsharded_params_total_size += (
                p_leaf.size
            )  # Add to total unsharded parameter size
            ...
            problematic_tensors_details.append(
                {
                    ...
                    "spec": str(
                        current_sharding_spec
                    ),  # Tensor sharding spec as string
                    "available_axes": sorted(
                        list(valid_target_mesh_axes)
...
def _raise_if_unsharded_exceeds_tolerance(
    unsharded_size, total_size, tolerance, problematic_tensors_details
):
    """
    ...
    if unsharded_param_perc > tolerance:
        ...
        for detail in problematic_tensors_details[
            :5
        ]:  # Show top 5 largest problematic tensors
            error_msg_lines.append(
...
def init_decode_state(apply_fn, params) -> train_state.TrainState:
    ...
    state = train_state.TrainState(
        step=0, apply_fn=apply_fn, params=params, tx=None, opt_state={}
    )  # type: ignore
    return state
...
def get_abstract_state(model, tx, config, rng, mesh, is_training=True):
    ...
    init_state_partial = functools.partial(
        init_initial_state, model, tx, config, is_training, rng
    )
    ...
    if is_training and config.optimizer_memory_host_offload:
        opt_state = jax.tree_util.tree_map(
            lambda x: x.with_memory_kind(kind="pinned_host"),
    ...
    if is_training and config.parameter_memory_host_offload:
        ...
        ), "You must set the scan axis 0 to enable parameter offloading."
    ...
    unboxed_abstract_sharded_state = max_utils.unbox_logicallypartioned(
...
def get_prefill_kv_cache_annotations(
    model, config, rng, mesh, page_state: None | PageState = None
):
    """Get a shaped abstraction of the state (including optimizer)"""
...
def logical_axis_rules_pp_act_as_dp(logical_rules):
    ...
    for key, physical_axes in logical_rules:
        ...
        if "data" in new_physical_axes:
            ...
            new_physical_axes = (
                new_physical_axes[0:data_idx]
                + ("stage",)
                + new_physical_axes[data_idx:]
            )
...
def create_device_mesh(config, devices=None):
    ...
    ici_parallelism = max_utils.fill_unspecified_mesh_axes(
        config.ici_parallelism.copy(), num_devices_per_slice, "ICI"
    )
    ...
    if multi_slice_env:
        dcn_parallelism = max_utils.fill_unspecified_mesh_axes(
            config.dcn_parallelism.copy(), num_slices, "DCN"
        )
        if max_utils.is_valid_custom_mesh(ici_parallelism, config.custom_mesh):
            mesh = max_utils.create_custom_device_mesh(
                ici_parallelism, dcn_parallelism, devices, config.custom_mesh
            )
        else:
...
def get_physical_spec_no_fsdp(full_logical, mesh, logical_axis_rules):
            ...
            if isinstance(named_sharding, jax.sharding.NamedSharding):
                ...
                for axis in named_sharding.spec:
                    if axis is None:
                        ...
                        new_axis = [
                            a for a in axis if a not in ("fsdp", "fsdp_transpose")
                        ]
                ...
                return jax.sharding.NamedSharding(
                    named_sharding.mesh, jax.sharding.PartitionSpec(*new_spec)
                )
    ...
    physical = nn.logical_to_mesh_sharding(
